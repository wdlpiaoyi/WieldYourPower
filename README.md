...
- 只拦**不走原版伤害链路**的代码：直接 `setHealth(X)`（`X` 低于当前血量）会被改写为 `floor((当前血量 - X) × 系数)`；**负数（含 `-Inf`）等价于 `setHealth(0)`**，同样按变化量算；直接 `die()` 也会被拦截回血。实体一旦进入死亡序列（`deathTime > 0` 或已 `dead`）就不再兜底回血，避免脚本化死亡动画被反复复活。走 `hurt` / `actualHurt` 的伤害（普通武器、OP 武器、生物攻击）一律不动。
